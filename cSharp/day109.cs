...
static int[] SearchCsvRows(
    ...
    for (int x = 1; x < rows.Count; x++)
    {
        ...
        bool rowIsMatch = partial
            ? row[columnIndex].Contains(value) // do something else
...
void SearchRoutine()
{
    ...
    int[] rowIds1 = SearchCsvRows("FirstName", query1, true);
    int[] rowIds2 = SearchCsvRows("LastName", query1, true);
    int[] rowIds3 = SearchCsvRows("Email", query1, true);
