...
)
{
    ...
    int tableWidth = totalLength + (4 * rowMaxLengths.Length);
    ...
    )
    {
        ...
        )
        {
            ...
            int padding = rowMaxLengths[index]
                - cell.Length;
            ...
            if (rowId == 0)
            {
                ConsoleWriteWithColor(cell, ConsoleColor.Green);
            }
            else
            {
                Console.Write(cell);
...
void UpdateRowRoutine()
{
    Console.WriteLine("What rowId do you want to update");
    string rowIdInput = Console.ReadLine();

    int.TryParse(rowIdInput, out int rowId);

    string[] row = ReadCsvRow(rowId);

    string email = UpdateAskForField("Email", row[0]);

    string firstName = UpdateAskForField("FirstName", row[1]);

    string lastName = UpdateAskForField("LastName", row[2]);
